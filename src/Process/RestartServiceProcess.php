...
namespace Captainbi\Hyperf\Process;
    ...
    public function handle(): void
    {
        ...
        while (true) {
            if (false !== @$this->process->pop()) {
                ...
                $script = trim(strval($config->get('restart_console.script_path', BASE_PATH . '/bin/restart.php')));
                ...
                $accessToken = trim(strval($config->get('restart_console.dingtalk_token', '')));
                ...
                $dingTalkSecret = trim(strval($config->get('restart_console.dingtalk_secret', '')));
                ...
                $message = base64_encode(strtr(trim(strval($config->get(
                    'restart_console.dingtalk_message',
                ...
                $timeout = min(60, max(3, (int)$config->get('restart_console.timeout', 60)));
