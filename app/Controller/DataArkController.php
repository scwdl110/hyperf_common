...
class DataArkController extends AbstractController
{
    ...
    protected $tableMap = [
        'erp_finance_{DBHOST}.f_dw_channel_day_report' => [
            'presto' => 'dwd.dwd_dataark_f_dw_channel_day_report_{DBHOST}',
            'es' => 'f_dw_channel_day_report_{DBHOST}',
        ],
        'erp_finance_{DBHOST}.f_dw_channel_week_report' => [
            'presto' => 'dwd.dwd_dataark_f_dw_channel_week_report_{DBHOST}',
            'es' => '',
        ],
        'erp_finance_{DBHOST}.f_dw_channel_month_report' => [
            'presto' => 'dwd.dwd_dataark_f_dw_channel_month_report_{DBHOST}',
            'es' => '',
        ],
        'erp_base.b_department_channel' => [
            'presto' => 'dim.dim_dataark_b_department_channel',
            'es' => '',
        ],
        'erp_base.b_user_department' => [
            'presto' => 'ods.ods_dataark_b_user_department',
            'es' => '',
        ],
        'erp_base.b_user_channel' => [
            'presto' => 'dim.dim_dataark_b_user_channel',
            'es' => '',
        ],
        'erp_base.b_site_rate' => [
            'presto' => 'ods.ods_dataark_b_site_rate',
            'es' => '',
        ],
        'erp_base.b_channel' => [
            'presto' => 'ods.ods_dataark_b_channel',
            'es' => '',
        ],
        'erp_finance_{DBHOST}.f_amazon_fba_inventory_by_channel_{CODENO}' => [
            'presto' => 'ods.ods_dataark_f_amazon_fba_inventory_by_channel_{DBHOST}',
            'es' => '',
        ],
        'erp_goods_{DBHOST}.g_amazon_goods_tags_rel_{CODENO}' => [
            'presto' => 'ods.ods_dataark_g_amazon_goods_tags_rel_{DBHOST}',
            'es' => '',
        ],
        'erp_goods_{DBHOST}.g_amazon_goods_tags_{CODENO}' => [
            'presto' => 'ods.ods_dataark_g_amazon_goods_tags_{DBHOST}',
            'es' => '',
        ],
        'erp_finance_{DBHOST}.f_amazon_goods_finance_{CODENO}' => [
            'presto' => 'ods.ods_dataark_f_amazon_goods_finance_{DBHOST}',
            'es' => '',
        ],
        'erp_finance_{DBHOST}.f_amazon_goods_isku_{CODENO}' => [
            'presto' => 'ods.ods_dataark_f_amazon_goods_isku_{DBHOST}',
            'es' => '',
        ],
    ];
    ...
    protected function getListByUnGoods(
        ...
        if ($count_tip == 2) { //仅统计总条数
            $count = $this->getTotalNum($where, $table_group, $table, $group);
            if($limit_num > 0 && $count > $limit_num){
                $count = $limit_num ;
            }
        } else if ($count_tip == 1) {  //仅仅统计列表
            if ($params['is_count'] == 1){
                $where = $this->getLimitWhere($where,$params,$table_group,$table,$limit,$orderby,$group);
                $lists = $this->select($where, $field_data, $table_group, $table, $limit);
            }else{
                $lists = $this->select($where, $field_data, $table_group, $table, $limit, $orderby, $group);
    ...
    private function getTotalNum($where = '', $table_group = '', $table = '', $group = '')
    {
        return $this->count($where, $table_group, $table, $group);
    }
     ...
     * @return string
     */
    protected function getLimitWhere($where,$datas,$table_group,$table,$limit,$orderby,$group){
    ...
    protected function getUnGoodsFbaData($lists = [], $fields = [], $datas = [], $channel_arr = [], $currencyInfo, $exchangeCode)
    {
        ...
        $amazon_fba_inventory_by_channel_md = base::load_model_class("amazon_fba_inventory_by_channel_pg","datas_ark") ;
        $where.= ' AND ' . $where_str ;
        if ($datas['currency_code'] == 'ORIGIN') {
     ...
     * @author: LWZ
     */
    protected function getListByGoods($where = '', $datas = array(), $limit = '', $sort = '', $order = '', $count_tip = 0,$channel_arr = array(), $currencyInfo, $exchangeCode, $searchType)
    {
        //没有按周期统计 ， 按指标展示
        if ($datas['show_type'] == 2) {
            $fields = $this->getGoodsFields($datas);
        } else {
            $fields = $this->getGoodsTimeFields($datas);
        }
        if (empty($fields)) {
            return array();
        }
        ...
        if(($datas['count_periods'] == 0 || $datas['count_periods'] == 1) && $datas['cost_count_type'] != 2){ //按天或无统计周期
            if ($searchType === self::SEARCH_TYPE_ES) {
                $table = "f_dw_goods_day_report_{$this->user['dbhost']} AS report" ;
            } else {
                $table = "dwd.dwd_dataark_f_dw_goods_day_report_{$this->user['dbhost']} AS report" ;
            }
        }else if($datas['count_periods'] == 2 && $datas['cost_count_type'] != 2){  //按周
            $table = "dwd.dwd_dataark_f_dw_goods_week_report_{$this->user['dbhost']} AS report" ;
        }else if($datas['count_periods'] == 3 || $datas['count_periods'] == 4 || $datas['count_periods'] == 5 ){
            $table = "dwd.dwd_dataark_f_dw_goods_month_report_{$this->user['dbhost']} AS report" ;
        ...
        if (in_array($datas['count_dimension'], ['parent_asin', 'asin', 'sku'])) {
            ...
            $table.= " LEFT JOIN ods.ods_dataark_g_amazon_goods_tags_rel_{$this->user['dbhost']} AS tags_rel ON tags_rel.goods_id = report.goods_g_amazon_goods_id and  tags_rel.status = 1 LEFT JOIN ods.ods_dataark_g_amazon_goods_tags_{$this->user['dbhost']} AS gtags ON gtags.id = tags_rel.tags_id AND gtags.status = 1" ;
            if ($datas['count_periods'] > 0 && $datas['show_type'] == '2') {
                if ($datas['count_periods'] == '1' ) { //按天
        ...
        if (!empty($where_detail)) {
            ...
            if (!empty($where_detail['tag_id'])) {
                if (strpos($group, 'tags_rel.tags_id') === false) {
                    $table .= " LEFT JOIN ods.ods_dataark_g_amazon_goods_tags_rel_{$this->user['dbhost']} AS tags_rel ON tags_rel.goods_id = report.goods_g_amazon_goods_id LEFT JOIN ods.ods_dataark_g_amazon_goods_tags_{$this->user['dbhost']} AS gtags ON gtags.id = tags_rel.tags_id";
                }
                if(is_array($where_detail['group_id'])){
        ...
        if ($count_tip == 2) { //仅统计总条数
            ...
            if ($datas['is_count'] == 1){
                $where = $this->getLimitWhere($where,$datas,$table_group,$table,$limit,$orderby,$group);
                $lists = $this->select($where, $field_data, $table_group, $table);
            }else{
                $lists = $this->select($where, $field_data, $table_group, $table, $limit, $orderby, $group);
...
//            $where = 'g.user_id = ' . $lists[0]['user_id']  . ' AND is_parent = 0 ' ;
            ...
            $table = "ods.ods_dataark_f_amazon_goods_finance_{$this->user['dbhost']} as g " ;
            if($datas['count_dimension'] == 'sku'){
                ...
                $table .= "  LEFT JOIN ods.ods_dataark_g_amazon_goods_tags_rel_{$this->user['dbhost']} AS rel ON g.g_amazon_goods_id = rel.goods_id ";
            }else if($datas['count_dimension'] == 'head_id') { //负责人
                $fba_fields = $group = 'i.head_id ,g.fba_inventory_v3_id' ;
                $table .= "  LEFT JOIN ods.ods_dataark_f_amazon_goods_isku_{$this->user['dbhost']} AS i ON g.isku_id = i.id  ";
        ...
        $goods_finance_md = base::load_model_class("amazon_goods_finance_pg","datas_ark") ;
